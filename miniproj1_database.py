...
#connect to mongodb
client= pymongo.MongoClient("mongodb://<dbuser>:<dbpassword>@ds153352.mlab.com:53352/ec601_database")
db = client['ec601_database']   
coll = db["user"] 
#print(coll.find_one())
...
def download_images(api, username, number, folder):
    ...
    data= {'twt_id':username,'log':imgLog}
    coll.insert(data)
    print("Data stored!")
...
def labelPics(path):
    ...
    #store label information into mysql   
    cursor=db.cursor()
    ...
    for label in labels:
        try:
            ...
            db.rollback()
    #print("Data stored into mysql!")
    
    #store label information into mongodb
    for label in labels:
        data1={'twt_id':username,'labels':label.description} 
        coll.insert(data1)
    print('Data stored into mongo!')
